...
struct InstanceData {
    vec4 corner0; // (tl.x, tl.y, tr.x, tr.y)
    vec4 corner1; // (br.x, br.y, bl.x, bl.y)
    ivec4 clip;
    vec4 clipRadii;
    vec2 pos;
    vec2 size;
...
void main(){
    int iid = gl_VertexID / 6; // instance index
    int vid = gl_VertexID % 6; // vertex within the quad
    ...
    // Unpack per-corner colors from packed RGBA8
