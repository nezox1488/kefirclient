    ...
    public void tick() {
        ...
        if (isJumping) {
            ...
            if (inWater && (!onGround || fluidLevel > swimHeight)) {
                ...
                jump();
                if (player.equals(mc.player) && (!KefirDLC.getInstance().getFunctionManager().getModule(NoDelay.class).isToggled() || !KefirDLC.getInstance().getFunctionManager().getModule(NoDelay.class).Jump.getValue())) {
                    jumpingCooldown = 10;
                }
            }
        }
