...
public class PotionsScreen extends HudElementScreen {
    ...
    private static final Identifier POTIONS_ICON = Identifier.of("kefir", "HudIcon/potions.png");
    ...
    public void render(Renderer2D renderer, int alpha, int blurAlpha, List<PotionLine> potions) {
        ...
        float iconY = y + 4f;
        if (MinecraftClient.getInstance().getTextureManager().getTexture(POTIONS_ICON).getGlTexture() instanceof GlTexture glTexture) {
